...
app.use(
  cors({
    origin: "https://silly-bell-74b3d0.netlify.app",
...
app.post("/api/kategori_bireysel/olustur", function (req, res) {
  if (req.isAuthenticated() && req.user.rol === "admin") {
    ...
    res.send({ sonuc: "Giriş yapmanız gerekiyor." });
  }
});
...
const miktariBul = function (id, items) {
  var miktar = 0;
...
app.post("/create-payment-intent", async (req, res) => {
  ...
  var toplam = 0;
  var urunlerinIdDegeleri = [];

  console.log(items);

  await items.forEach((element) => {
    urunlerinIdDegeleri.push(element.id);
  });

  await Urun.find({ _id: { $in: urunlerinIdDegeleri } })
    .exec()
    .then(function (gelenVeri) {
      gelenVeri.forEach((element) => {
        console.log("fiyat", element.ind_fiyat);
        console.log("miktar", miktariBul(element.id, items));
        toplam +=
          parseInt(element.ind_fiyat * 100) * miktariBul(element.id, items);
