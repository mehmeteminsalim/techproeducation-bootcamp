...
app.get("/kitap/:isim/:index1", function(req, res){
    var indexDegeri = req.params.index1;
...
app.get("/kitap/:kategoriAdi",(req,res)=>{
  
  var kategoriName = req.params.kategoriAdi;
  var kategoriDizisi = [];
  for (let i = 0; i < ucTaneKitap.length; i++) {
    
    if (ucTaneKitap[i].kategori == kategoriName) {
      kategoriDizisi.push(ucTaneKitap[i]);
    }
    
  }
  res.render("kategori",{
    gelenKategori : kategoriDizisi
  })
});
